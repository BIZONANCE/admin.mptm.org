  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      if (event.target?.result) {
        setFormData((prev) => ({ ...prev, imageUrl: String(event.target?.result) }));
      }
    };
          ...
          <div className="bg-white rounded-3xl max-w-2xl w-full overflow-hidden shadow-2xl space-y-4 p-6 border border-slate-200 my-auto">
            ...
            <form onSubmit={handleSaveAd} className="space-y-4 text-xs font-bold text-slate-700">
              ...
              <div className="space-y-2">
                <label className="block">Banner Image (URL or Upload) :</label>
                <div className="flex flex-col sm:flex-row items-center gap-3">
                  <input
                    ...
                    className="flex-1 w-full bg-slate-50 border border-slate-300 rounded-xl px-3 py-2 text-slate-900 outline-none focus:border-indigo-600 font-mono text-[11px]"
                  />
                  <label className="px-3 py-2 bg-slate-100 hover:bg-slate-200 border border-slate-300 text-slate-700 font-bold rounded-xl text-xs flex items-center gap-1.5 cursor-pointer whitespace-nowrap">
